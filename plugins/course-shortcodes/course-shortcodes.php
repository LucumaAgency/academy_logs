...
function instructor_position_shortcode($atts) {
    $atts = shortcode_atts([
        'user_id' => 0,
        'post_id' => 0, // Allow specifying stm-courses post ID
        'class' => 'instructor-position', // CSS class for the wrapper
    ...
    // Get the instructor's position from user meta
    // Note: Replace 'instructor_position' with the actual meta key if different
    $position = get_user_meta($user_id, 'instructor_position', true);
    if (empty($position)) {
        return esc_html__('No position available', 'course-shortcodes');
    }
