...
// Include webinar price shortcode
require_once __DIR__ . '/shortcode-webinar-price.php';
...
/**
 * SYNC CURRICULUM FROM MASTERSTUDY LMS WITH CURRICULUM POST TYPE
 */

/**
 * Depuración: Registrar mensajes en el debug.log
 */
...
/**
 * Solucionar advertencias de ACF textdomain
 */
add_action('init', function() {
    log_debug('ACF textdomain init hook triggered');
...
/**
 * Confirmar que el código se carga
 */
log_debug('Custom functions.php loaded');

/**
 * Permitir que el CPT curriculum se use en el currículum de MasterStudy
 */
add_filter('stm_lms_curriculum_post_types', function($post_types) {
    if (!in_array('curriculum', $post_types)) {
        $post_types[] = 'curriculum';
    }
    log_debug('Curriculum post types filter: ' . print_r($post_types, true));
    return $post_types;
}, 10, 1);

/**
 * Sincronización bidireccional: Actualizar los metadatos de MasterStudy cuando se guardan los campos ACF
 */
function sync_acf_to_masterstudy($post_id) {
    log_debug("sync_acf_to_masterstudy triggered for post_id: $post_id");

    // Evitar bucles infinitos y verificar que es el CPT correcto
    if (get_post_type($post_id) !== 'curriculum' || defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) {
        log_debug("Skipping sync_acf_to_masterstudy for post_id $post_id: Not curriculum or autosave");
        return;
    }

    // Obtener el curso asociado
    $associated_course = get_field('associated_course', $post_id);
    if (!$associated_course) {
        log_debug("No associated course for curriculum post_id $post_id");
        return;
    }

    // Manejar el caso de que associated_course sea un array o un ID
    $course_id = is_array($associated_course) ? $associated_course[0]->ID : $associated_course;
    log_debug("Associated course ID for curriculum post_id $post_id: $course_id");

    // Obtener el currículum del curso
    $curriculum = get_post_meta($course_id, 'stm_course_curriculum', true);
    $curriculum_items = $curriculum ? explode(',', $curriculum) : [];

    // Asegurarse de que la entrada de curriculum esté en el curso
    if (!in_array($post_id, $curriculum_items)) {
        $curriculum_items[] = $post_id;
        update_post_meta($course_id, 'stm_course_curriculum', implode(',', $curriculum_items));
        log_debug("Added curriculum post_id $post_id to course $course_id curriculum");
    }

    // Obtener los valores de los campos ACF
    $short_description = get_field('lesson_short_description', $post_id);
    $lesson_content = get_field('lesson_content', $post_id);
    log_debug("ACF fields for post_id $post_id - Short description: " . ($short_description ?: 'empty') . ", Content: " . ($lesson_content ?: 'empty'));

    // Actualizar los metadatos de MasterStudy
    if ($short_description) {
        update_post_meta($post_id, 'lesson_excerpt', wp_kses_post($short_description));
        log_debug("Updated lesson_excerpt for post_id $post_id: $short_description");
    } else {
        delete_post_meta($post_id, 'lesson_excerpt');
        log_debug("Deleted lesson_excerpt for post_id $post_id");
    }

    if ($lesson_content) {
        // Actualizar el contenido principal de la entrada
        $post_data = [
            'ID'           => $post_id,
            'post_content' => wp_kses_post($lesson_content),
        ];
        remove_action('acf/save_post', 'sync_acf_to_masterstudy', 20);
        wp_update_post($post_data);
        add_action('acf/save_post', 'sync_acf_to_masterstudy', 20, 1);
        log_debug("Updated post_content for post_id $post_id");
    } else {
        $post_data = [
            'ID'           => $post_id,
            'post_content' => '',
        ...
        wp_update_post($post_data);
        add_action('acf/save_post', 'sync_acf_to_masterstudy', 20, 1);
        log_debug("Cleared post_content for post_id $post_id");
    }
}
add_action('acf/save_post', 'sync_acf_to_masterstudy', 20, 1);

/**
 * Sincronización bidireccional: Actualizar los campos ACF cuando se guardan los datos desde el dashboard de MasterStudy
 */
function sync_masterstudy_to_acf($lesson_id, $data) {
    log_debug("sync_masterstudy_to_acf triggered for lesson_id: $lesson_id, data: " . print_r($data, true));

    // Verificar que es el CPT correcto
    if (get_post_type($lesson_id) !== 'curriculum') {
        log_debug("Skipping sync_masterstudy_to_acf for lesson_id $lesson_id: Not curriculum");
        return;
    }

    // Obtener el curso asociado
    $associated_course = get_field('associated_course', $lesson_id);
    if (!$associated_course) {
        log_debug("No associated course for lesson_id $lesson_id");
        return;
    }

    // Manejar el caso de que associated_course sea un array o un ID
    $course_id = is_array($associated_course) ? $associated_course[0]->ID : $associated_course;
    log_debug("Associated course ID for lesson_id $lesson_id: $course_id");

    // Verificar que la lección pertenece al curso correcto
    $curriculum = get_post_meta($course_id, 'stm_course_curriculum', true);
    $curriculum_items = $curriculum ? explode(',', $curriculum) : [];
    if (!in_array($lesson_id, $curriculum_items)) {
        log_debug("Lesson_id $lesson_id not in course $course_id curriculum");
        return;
    }

    // Obtener los datos del formulario (ajustado para posibles nombres de campos)
    $short_description = isset($data['lesson_excerpt']) ? sanitize_textarea_field($data['lesson_excerpt']) : 
                        (isset($data['excerpt']) ? sanitize_textarea_field($data['excerpt']) : '');
    $lesson_content = isset($data['content']) ? wp_kses_post($data['content']) : 
                     (isset($data['lesson_content']) ? wp_kses_post($data['lesson_content']) : '');
    log_debug("Dashboard data for lesson_id $lesson_id - Short description: " . ($short_description ?: 'empty') . ", Content: " . ($lesson_content ?: 'empty'));

    // Actualizar los campos ACF
    update_field('lesson_short_description', $short_description, $lesson_id);
    log_debug("Updated lesson_short_description for lesson_id $lesson_id: " . ($short_description ?: 'empty'));

    update_field('lesson_content', $lesson_content, $lesson_id);
    log_debug("Updated lesson_content for lesson_id $lesson_id: " . ($lesson_content ?: 'empty'));
}

// Intentar ambos ganchos por si stm_lms_save_lesson no funciona
add_action('stm_lms_save_lesson', 'sync_masterstudy_to_acf', 10, 2);
add_action('stm_lms_lesson_updated', 'sync_masterstudy_to_acf', 10, 2);

/**
 * Modificar el formulario del dashboard para incluir entradas de curriculum
 */
add_filter('stm_lms_lesson_data', function($lesson_data, $lesson_id, $course_id) {
    log_debug("stm_lms_lesson_data filter triggered for lesson_id: $lesson_id, course_id: $course_id");
    if (get_post_type($lesson_id) === 'curriculum') {
        $lesson_data['lesson_excerpt'] = get_field('lesson_short_description', $lesson_id) ?: '';
        $lesson_data['content'] = get_field('lesson_content', $lesson_id) ?: '';
        log_debug("Loaded lesson data for lesson_id $lesson_id in course $course_id: " . print_r($lesson_data, true));
    }
    return $lesson_data;
}, 10, 3);
